// TAG — PURSUIT v2
// Pure JavaScript entry point — loads Three.js then boots the game
...
.sgrid{display:grid;grid-template-columns:repeat(4,168px);gap:.65rem;margin:.45rem 0;max-height:52vh;overflow-y:auto;padding-right:.4rem}
...
<div class="scr hide" id="s-mode">
  ...
  <div class="modecards">
    <div class="modecard casual" id="mc-casual">
      <div class="modeicon">🏃</div>
      <div class="modename">CASUAL</div>
      <div class="modedesc">3-minute match vs bots.<br>Chill, no rank on the line.<br>Earn coins &amp; skills.</div>
    </div>
    <div class="modecard ranked" id="mc-ranked">
      <div class="modebadge hot">HOT</div>
      <div class="modeicon">🏆</div>
      <div class="modename">RANKED</div>
      <div class="modedesc">30-second sprint matches.<br>Earn / lose rank points.<br>Climb the ladder.</div>
    </div>
    <div class="modecard multi" id="mc-multi">
      <div class="modebadge new">NEW</div>
      <div class="modeicon">👥</div>
      <div class="modename">MULTIPLAYER</div>
      <div class="modedesc">3-5 real players.<br>Share a room code.<br>Works in-browser.</div>
    </div>
    <div class="modecard prac" id="mc-prac">
      <div class="modeicon">🎯</div>
      <div class="modename">PRACTICE</div>
      <div class="modedesc">Solo arena. No bots.<br>Learn the maps.<br>Test your skills.</div>
    </div>
  </div>
...
<div id="s-hud">
  ...
  <div id="hskills">
    <div class="ss">
      <span class="ssl" id="sqbind">Q</span>
      <span class="ssn" id="sqn">SLIDE</span>
      <div class="sso" id="sqo" style="height:0"></div>
      <span class="sst" id="sqt"></span>
    </div>
    <div class="ss">
      <span class="ssl" id="sebind">E</span>
      <span class="ssn" id="sen">—</span>
      <div class="sso" id="seo" style="height:0"></div>
      <span class="sst" id="set2"></span>
    </div>
  </div>
  <div id="mphud"></div>
  <div id="chatw"></div>
...
// CONSTANTS
// ═══════════════════════════════════════════
const PH=1.8,SH=0.8,PR=0.42,GRV=-22,MSZ=1620,HM=810;
const SPD=30,SSPD=40; // High speed
const JF=9.5,TAGD=2.5,TAG_COOLDOWN=1.0; // 1s tag cooldown
const DUR_CASUAL=180,DUR_RANKED=30,DUR_PRACTICE=120;
const BSPD=30; // Bots same speed as player
...
function getRank(rp){
  let r=RANKS[0];
  for(const rk of RANKS)if(rp>=rk.min)r=rk;
  return r;
}
function getRankProgress(rp){
  const ri=RANKS.findLastIndex(r=>rp>=r.min);
  if(ri>=RANKS.length-1)return 1;
  const cur=RANKS[ri],nxt=RANKS[ri+1];
  return Math.min(1,(rp-cur.min)/(nxt.min-cur.min));
}
...
function genObs(md){
  ...
  const {p:pt,s:sr,d:baseDens}=md;

  // FIXED RAW TRAP COUNTS — not density-scaled, always this many objects
  // Each layout gets its own raw number regardless of map size.
  // These numbers are 10x what the original had.
  const TRAP_COUNTS={
    grid:600, corridors:450, ring:500, scattered:700,
    maze:600, pillars:550, platforms:500, compound:750
  };
  const N = TRAP_COUNTS[pt] || 500; // Capped for performance

  if(pt==='grid'){
    // Core grid every 18 units across massive map
    ...
    // N random extra traps scattered everywhere
    for(let i=0;i<N;i++){
    ...
    for(let row=0;row<Math.floor(MSZ/cell);row++)for(let col=0;col<Math.floor(MSZ/cell);col++){
      ...
      if(r()<.52){const h=3+r()*4;
        const t=r()<.12?'web':r()<sr?'slide':'wall';
        ...
        add(x,0,z,cell-1,3+r()*4,2,'wall',th.w);
      }
      // Additional scattered traps inside each cell
    ...
    // N extra traps and webs between pillars
    for(let i=0;i<N;i++){
    ...
    for(let i=0;i<N;i++){
      ...
      if(elev>0){
        add(x,0,z,1.2,elev,1.2,'wall',th.w);
        add(x,elev,z,6+r()*8,.6,6+r()*8,'platform',th.aw);
      } else {
        const t=r()<.18?'web':r()<sr?'jump':'wall';
...
const SDEFS=[
  ...
  {id:'smokeq',      slot:'q',name:'Smoke Screen',       desc:'Blind nearby IT for 4s in smoke cloud',                  baseCD:20,  cost:30},
  {id:'wallrun',     slot:'q',name:'Wall Run',           desc:'Sprint along walls for 2s without falling',              baseCD:22,  cost:35},
  ...
  {id:'phantomstep', slot:'q',name:'Phantom Step',       desc:'Leave 3 fake footstep trails to confuse IT',             baseCD:18,  cost:30},
  {id:'turbo',       slot:'q',name:'Turbo Wheels',       desc:'+90% speed for 5s with reduced turn radius',             baseCD:25,  cost:45},
  ...
  {id:'stasis',      slot:'q',name:'Stasis Bubble',      desc:'Freeze yourself in place — immune to tag for 1.5s',      baseCD:18,  cost:38},
  ...
  {id:'inviswall',   slot:'q',name:'Invis Wall',         desc:'Place invisible wall 6m ahead — blocks chasers 3s',      baseCD:25,  cost:48},
  ...
  {id:'hijump',      slot:'e',name:'Super Jump',         desc:'3× jump height',                                         baseCD:15,  cost:32},
  ...
  {id:'doppel',      slot:'e',name:'Doppelganger',       desc:'Spawn decoy that runs in your exact direction for 3s',   baseCD:28,  cost:55},
  ...
  {id:'superdive',   slot:'e',name:'Super Dive',         desc:'Dive at 3× normal speed, create shockwave on land',     baseCD:15,  cost:35},
  {id:'jailbreak',   slot:'e',name:'Jailbreak',          desc:'If caught as IT: transfer IT to nearest bot instantly',  baseCD:40,  cost:72},
  ...
  {id:'echo',        slot:'e',name:'Echo Dash',          desc:'Dash 4× in rapid succession, each 5m',                  baseCD:20,  cost:48},
  ...
  {id:'rocketjump',  slot:'e',name:'Rocket Jump',        desc:'Explosive jump — 2× height, leaves blast crater',       baseCD:16,  cost:38},
...
function doSave(){
  localStorage.setItem('tagpursuit2',JSON.stringify({
    ...
    rp:player.rankPoints,binds:{q:BINDS.q,e:BINDS.e},lb:leaderboard
  }));
}
...
let tagCooldownTimer=0; // Global tag cooldown

const player={
  x:0,y:0,z:3,vx:0,vy:0,vz:0,
  yaw:0,pitch:0,
  isIt:false,isGrounded:false,isSliding:false,isDiving:false,
  slideTimer:0,diveTimer:0,tagImmunity:0,
  wallNX:0,wallNZ:0,wallJumpCd:0,
  bobTime:0,itTime:0,tagsMade:0,
  ...
  // Skill effect timers (all initialized to 0/false/null)
...
// Multiplayer simulated players (local sim for "real player" feel)
const HUMAN_NAMES=['Apex','Raven','Nyx','Striker','Ghost','Rift','Bolt','Cipher'];
let multiPlayers=[];

const bots=[
  {id:0,name:'Shadow',x:225, y:0,z:0, vx:0,vy:0,vz:0,isIt:false,isGrounded:true,itTime:0,tagsMade:0,mesh:null,aiAng:0,  stuckT:0,stuckCk:0,pX:225, pZ:0,  tagIm:0},
  {id:1,name:'Blaze', x:-225,y:0,z:0, vx:0,vy:0,vz:0,isIt:false,isGrounded:true,itTime:0,tagsMade:0,mesh:null,aiAng:1.6,stuckT:0,stuckCk:0,pX:-225,pZ:0,  tagIm:0},
  {id:2,name:'Vex',   x:0,   y:0,z:-225,vx:0,vy:0,vz:0,isIt:false,isGrounded:true,itTime:0,tagsMade:0,mesh:null,aiAng:3.1,stuckT:0,stuckCk:0,pX:0,   pZ:-225,tagIm:0},
];

let leaderboard=[
  {name:'YOU',wins:0,losses:0,streak:0,rp:0},
];

let decoyMesh=null,itLight=null;
const chatMsgs=[];
let mapChoices=[];
...
const camera=new THREE.PerspectiveCamera(82,innerWidth/innerHeight,.05,7000);
...
function buildScene(map){
  ...
  scene.fog=new THREE.FogExp2(th.fog,(th.fe||.02)*0.12);
  const gm=new THREE.Mesh(new THREE.PlaneGeometry(MSZ+800,MSZ+800,100,100),sMat(th.gr,0,.92,.02));
  gm.rotation.x=-Math.PI/2;gm.receiveShadow=true;scene.add(gm);
  const gh=new THREE.GridHelper(MSZ,600,th.l1,th.w);gh.material.opacity=.10;gh.material.transparent=true;scene.add(gh);
  scene.add(new THREE.AmbientLight(th.sky,.55));
  const dl=new THREE.DirectionalLight(0xffffff,.95);dl.position.set(22,38,22);dl.castShadow=true;
  dl.shadow.mapSize.set(2048,2048);dl.shadow.camera.far=3600;dl.shadow.camera.left=dl.shadow.camera.bottom=-1800;dl.shadow.camera.right=dl.shadow.camera.top=1800;scene.add(dl);
  // scene.add(Object.assign(new THREE.PointLight(th.l1,2.5,1400),{position:new THREE.Vector3(-500,20,-500)}));
  // scene.add(Object.assign(new THREE.PointLight(th.l2,2.5,1400),{position:new THREE.Vector3(500,20,500)}));
  // scene.add(Object.assign(new THREE.PointLight(th.l1,2.0,1200),{position:new THREE.Vector3(500,20,-500)}));
  // scene.add(Object.assign(new THREE.PointLight(th.l2,2.0,1200),{position:new THREE.Vector3(-500,20,500)}));
  const light1 = new THREE.PointLight(th.l1, 2.5, 1400);
  light1.position.set(-500, 20, -500);
  scene.add(light1);

  const light2 = new THREE.PointLight(th.l2, 2.5, 1400);
  light2.position.set(500, 20, 500);
  scene.add(light2);

  const light3 = new THREE.PointLight(th.l1, 2.0, 1200);
  light3.position.set(500, 20, -500);
  scene.add(light3);

  const light4 = new THREE.PointLight(th.l2, 2.0, 1200);
  light4.position.set(-500, 20, 500);
  scene.add(light4);
  ...
  for(const obs of map.obs){
    ...
    m.position.set(obs.x,obs.mnY+obs.h*.5,obs.z);m.castShadow=obs.type!=='bound';m.receiveShadow=true;scene.add(m);
    ...
    else if(obs.type==='barrier'){const bi=new THREE.Mesh(bGeo(.07,obs.h,obs.d),new THREE.MeshStandardMaterial({color:0xff4400,emissive:0xff4400,emissiveIntensity:.7,transparent:true,opacity:.72}));bi.position.set(obs.mnX,obs.mnY+obs.h*.5,obs.z);scene.add(bi);}
    else if(obs.type==='web'){
      buildWebMesh(obs,scene);
    }
  }
  const botCount=gameMode==='multi'?0:3;
  bots.slice(0,botCount).forEach(b=>{if(b.mesh)scene.remove(b.mesh);b.mesh=mkBotMesh([0x4466ee,0xff7722,0x22ffbb][b.id]);b.mesh.position.set(b.x,b.y,b.z);scene.add(b.mesh);});
  // Multi player meshes
  multiPlayers.forEach(mp=>{if(mp.mesh)scene.remove(mp.mesh);mp.mesh=mkBotMesh(mp.color);mp.mesh.position.set(mp.x,mp.y,mp.z);scene.add(mp.mesh);});
...
function mkBotMesh(col){
  ...
  const body=new THREE.Mesh(bGeo(.62,1.2,.62),sMat(col,0,.72,.18));body.position.y=.9;body.castShadow=true;g.add(body);
  const head=new THREE.Mesh(new THREE.SphereGeometry(.3,8,6),sMat(col,0,.65,.12));head.position.y=1.72;head.castShadow=true;g.add(head);
  const eMat=new THREE.MeshStandardMaterial({color:0xffffff,emissive:0xffffff,emissiveIntensity:.55});
  [-1,1].forEach(s=>{const e=new THREE.Mesh(new THREE.SphereGeometry(.055,4,4),eMat);e.position.set(s*.12,.08,.26);head.add(e);});
  const ring=new THREE.Mesh(new THREE.TorusGeometry(.42,.05,6,16),new THREE.MeshStandardMaterial({color:0xff2040,emissive:0xff2040,emissiveIntensity:1.6}));
  ring.position.y=2.35;ring.name='itr';ring.visible=false;g.add(ring);
...
document.addEventListener('keydown',e=>{
  if(listenBind){
    ...
    listenBind=null;
    document.querySelectorAll('.kbkey').forEach(k=>k.classList.remove('listening'));
    doSave();return;
  }
...
function updatePlayer(dt){
  ...
  if(p.hauntedTimer>0){
    ...
    if(p._hauntCD<=0&&p.hauntedTimer>0){p._hauntCD=0.5;
      // Spawn a brief ghost decoy at current position
      const hg=new THREE.Mesh(new THREE.SphereGeometry(.4,6,5),
        new THREE.MeshStandardMaterial({color:0xaaaaff,emissive:0x8888ff,emissiveIntensity:.8,transparent:true,opacity:.5}));
      hg.position.set(p.x,p.y+.9,p.z);scene.add(hg);
      setTimeout(()=>scene.remove(hg),1500);
    }
  }
  ...
  if(p.homingTimer>0){p.homingTimer=Math.max(0,p.homingTimer-dt);const itb2=bots.find(b=>b.isIt&&!b.isIt===false);const tgt=!p.isIt?bots.find(b=>!b.isIt):bots.find(b=>b.isIt);if(tgt&&p.homingTimer>0){const dx=tgt.x-p.x,dz=tgt.z-p.z,dl=Math.hypot(dx,dz)||1;p.vx+=dx/dl*8;p.vz+=dz/dl*8;}}
  ...
  // Gravity flip: invert gravity effect
  if(p.gravFlip>0&&p.vy<0)p.vy=Math.abs(p.vy)*.5;
  // Moon walk: reduce gravity pull
  if(p.moonTimer>0)p.vy=Math.max(p.vy,-2);
  // Position history for rewind/timeloop
  ...
  // Slide via ShiftLeft OR Q (if Q is equipped as slide_q)
  const slideKey=shNow||(qNow&&p.equippedQ==='slide_q');
  if(slideKey&&!prevShift&&p.isGrounded&&!p.isSliding){
    p.isSliding=true;
    p.slideTimer=(p.sboostEff>0?2.5:1.6)*(1+p.upgrades.slide*.4);
  }
  if(p.isSliding){p.slideTimer-=dt;if(p.slideTimer<=0||(!slideKey&&p.isGrounded))p.isSliding=false;}

  if(ctNow&&!prevCtrl&&!p.isGrounded&&!p.isDiving){
    p.isDiving=true;p.diveTimer=.4;p.vy=-9;
    p.vx+=Math.sin(p.yaw)*14;p.vz+=Math.cos(p.yaw)*14;showStat('DIVE!');
  }
  ...
  if(spNow&&!prevSpace){
    ...
    else if((Math.abs(p.wallNX)>.5||Math.abs(p.wallNZ)>.5)&&p.wallJumpCd<=0){p.vy=jf*.88;p.vx=-p.wallNX*jf*.85;p.vz=-p.wallNZ*jf*.85;p.wallJumpCd=.35;showStat('WALL JUMP!');}
  }
  ...
  if(!p.phaseTimer||p.phaseTimer<=0){
    for(const o of curMap.obs){
      ...
      if(p.x>o.mnX&&p.x<o.mxX&&p.y<o.mxY&&p.y+PH>o.mnY&&p.z>o.mnZ&&p.z<o.mxZ){
        inWeb=true;
        p.vx*=0.18;p.vz*=0.18;          // heavy slowdown
        if(p.vy<0)p.vy*=0.12;           // resist falling — can climb
        p.isGrounded=false;              // no ground while in web
        if(spNow&&!prevSpace){
          // Delayed, weak jump out of web (1/5 normal height)
          setTimeout(()=>{if(gameRunning)p.vy=JF*0.2;},300);
        }
        if(!p._inWebLast)showStat('CAUGHT IN WEB!');
        break;
      }
    }
  }
  ...
  // Launchpad trigger
  for(const o of curMap.obs){
    if(o.type!=='launchpad')continue;
    if(p.x>o.mnX&&p.x<o.mxX&&p.y<=o.mxY+0.3&&p.z>o.mnZ&&p.z<o.mxZ){
      p.vy=40;p.isGrounded=false;showStat('LAUNCHED!');break;
    }
  }
  // Mine trigger (IT only — blows them back)
  for(const o of curMap.obs){
    if(o.type!=='mine')continue;
    if(Math.hypot(p.x-o.x,p.z-o.z)<2.5&&Math.abs(p.y-o.mnY)<1.5){
...
function activateSkill(slot){
  ...
  switch(sid){
    ...
    case'webshot':{const wx=p.x+Math.sin(p.yaw)*-10,wz=p.z+Math.cos(p.yaw)*-10;const fakeWeb={x:wx,yb:0,z:wz,w:4,h:3,d:4,type:'web',color:0x885599,mnX:wx-2,mxX:wx+2,mnY:0,mxY:3,mnZ:wz-2,mxZ:wz+2};curMap.obs.push(fakeWeb);buildWebMesh(fakeWeb);setTimeout(()=>{const idx=curMap.obs.indexOf(fakeWeb);if(idx>-1)curMap.obs.splice(idx,1);},4000);showStat('WEB FIRED!');break;}
    case'sprint':p.sprintEff=1;showStat('SPRINT BURST!');break;
    case'feint':p.vx*=-1.5;p.vz*=-1.5;showStat('FEINT!');break;
    case'grapple':{let bd=999,bx=p.x,bz=p.z;for(const o of curMap.obs){if(o.type==='bound')continue;const cd=Math.hypot(o.x-p.x,o.z-p.z);if(cd<bd&&cd<60){bd=cd;bx=o.x;bz=o.z;}}if(bd<60){const dx=bx-p.x,dz=bz-p.z,dl=Math.hypot(dx,dz);p.vx=dx/dl*45;p.vz=dz/dl*45;p.vy=8;showStat('GRAPPLE!');}break;}
    ...
    case'webtrap':{const ftw={x:p.x,yb:0,z:p.z,w:4,h:3,d:4,type:'web',color:0x885599,mnX:p.x-2,mxX:p.x+2,mnY:0,mxY:3,mnZ:p.z-2,mxZ:p.z+2};curMap.obs.push(ftw);buildWebMesh(ftw);setTimeout(()=>{const idx=curMap.obs.indexOf(ftw);if(idx>-1)curMap.obs.splice(idx,1);},5000);showStat('WEB DEPLOYED!');break;}
    ...
    case'zipline':{const farthest=HM*0.9;const ang=p.yaw;p.x+=Math.sin(ang)*-farthest*0.3;p.z+=Math.cos(ang)*-farthest*0.3;p.vx=Math.sin(ang)*-50;p.vz=Math.cos(ang)*-50;showStat('ZIPLINE!');break;}
    ...
    case'phantomstep':for(let pi=0;pi<3;pi++)setTimeout(()=>{const ph={x:p.x+(Math.random()-.5)*20,yb:0,z:p.z+(Math.random()-.5)*20,w:1,h:2,d:1,type:'bound',color:0xffff00,mnX:p.x-10,mxX:p.x+10,mnY:0,mxY:2,mnZ:p.z-10,mxZ:p.z+10};},pi*300);showStat('PHANTOM STEP!');break;
    ...
    case'mimic':{
      const itbm=bots.find(b=>b.isIt);
      if(itbm&&itbm._lastSkill){
        // Find the skill in SDEFS and activate it directly
        const mimicSid=itbm._lastSkill;
        const mimicDef=SDEFS.find(s=>s.id===mimicSid);
        if(mimicDef){
          // Temporarily pretend we have it equipped in E slot
          const oldE=player.equippedE;player.equippedE=mimicSid;
          player.skill2CD=0;activateSkill('e');player.equippedE=oldE;
          showStat('MIMIC: '+mimicDef.name.toUpperCase()+'!');
        }
      } else showStat('NOTHING TO MIMIC');
      break;}
    ...
    case'banish':{const b=bots.find(b=>b.isIt);if(b){b.x=(Math.random()>.5?1:-1)*HM*.88;b.z=(Math.random()>.5?1:-1)*HM*.88;showStat('BANISHED!');}break;}
    case'shadowstep':{const sdx=Math.sin(p.yaw)*-6,sdz=Math.cos(p.yaw)*-6;p.x+=sdx;p.z+=sdz;showStat('SHADOW STEP!');break;}
    ...
    case'webmine':{const wm2={x:p.x,yb:0,z:p.z,w:3,h:2,d:3,type:'web',color:0x885599,mnX:p.x-1.5,mxX:p.x+1.5,mnY:0,mxY:2,mnZ:p.z-1.5,mxZ:p.z+1.5};curMap.obs.push(wm2);buildWebMesh(wm2);setTimeout(()=>{const idx=curMap.obs.indexOf(wm2);if(idx>-1)curMap.obs.splice(idx,1);},6000);showStat('WEB MINE!');break;}
    ...
    case'wormhole':{
      if(!p._wormA){
        p._wormA={x:p.x,z:p.z};
        // Place a glowing portal marker
        const wm=new THREE.Mesh(new THREE.TorusGeometry(2,.2,8,32),
          new THREE.MeshStandardMaterial({color:0x00ffcc,emissive:0x00ffcc,emissiveIntensity:2}));
        wm.rotation.x=Math.PI/2;wm.position.set(p.x,.5,p.z);scene.add(wm);
        p._wormAMesh=wm;showStat('PORTAL A SET — USE AGAIN TO WARP!');
      } else {
        if(p._wormAMesh)scene.remove(p._wormAMesh);
        const wa=p._wormA;p._wormA=null;p._wormAMesh=null;
        p.x=wa.x;p.y=1;p.z=wa.z;p.vx=p.vy=p.vz=0;showStat('WORMHOLE!');
      }
      break;}
...
function updateBots(dt){
  ...
  for(const b of activeBots){
    ...
    if(b.slowTimer>0)bspd2*=0.4;   // blizzard slow
    if(b.siphoned>0)bspd2*=0.6;    // siphon drain
    if(b.dazed>0){mvX*=-1;mvZ*=-1;}// daze: invert direction
    if(b.tethered>0){mvX=0;mvZ=0;} // tether: stop
    if(b.anchored>0){mvX=0;mvZ=0;} // anchor: stop
    if(b.freezeTimer>0){mvX=0;mvZ=0;bspd2=0;} // freeze
  ...
  if(gameMode==='multi')updateMultiSim(dt);
}

// Simulate other players in multiplayer (smooth random movement, human-like)
function updateMultiSim(dt){
  for(const mp of multiPlayers){
    ...
    if(mp.aiTimer<=0){
      ...
      if(mp.isIt&&itEnt){
        const t=itEnt===player?player:itEnt;
        const ang=Math.atan2(t.x-mp.x,t.z-mp.z);
        mp.aiAng=ang+(Math.random()-.5)*.3;
      } else if(itEnt){
        const ang=Math.atan2(mp.x-itEnt.x,mp.z-itEnt.z);
        mp.aiAng=ang+(Math.random()-.5)*.5;
      } else {
        mp.aiAng+=(Math.random()-.5)*1.2;
      }
    }
...
// ═══════════════════════════════════════════
// TAG SYSTEM (1s cooldown)
// ═══════════════════════════════════════════
function checkTag(){
  // Global tag cooldown display
  const cdEl=document.getElementById('tagcd');
  const cdFill=document.getElementById('tagcdfill');
  if(player.isIt&&tagCooldownTimer>0){
    cdEl.style.display='block';
    cdFill.style.width=((1-tagCooldownTimer/TAG_COOLDOWN)*100)+'%';
  } else {
  ...
  if(player.isIt&&player.ghostEff<=0&&tagCooldownTimer<=0){
    for(const b of allTargets){
      ...
      if(Math.hypot(player.x-b.x,player.z-b.z)<TAGD){
        ...
        player.tagsMade++;
        showStat('TAGGED '+b.name.toUpperCase()+'!');addChat(`${b.name} is now IT!`,'sys');doShake(.4);
  ...
  if(itBot&&itBot.tagIm<=0){
    if(player.tagImmunity<=0&&player.ghostEff<=0&&Math.hypot(itBot.x-player.x,itBot.z-player.z)<TAGD){
      ...
      if(player.voltTimer>0){
        // Shock the IT bot back
        const dx=itBot.x-player.x,dz=itBot.z-player.z,dl=Math.hypot(dx,dz)||1;
        itBot.vx+=dx/dl*30;itBot.vz+=dz/dl*30;itBot.stunTimer=(itBot.stunTimer||0)+2;
        showStat('VOLT SHOCKED!');return;
      }
...
// RANKED ROUND SYSTEM
// ═══════════════════════════════════════════
let rankedScore={player:0,bots:[0,0,0]};

function awardRankedTag(playerTagged){
  // Tagger gets +10 RP per tag in ranked
  if(playerTagged){
    player.rankPoints=Math.max(0,player.rankPoints-5);
    showRankedResult('TAGGED! -5 RP','rdn');
  } else {
    player.rankPoints+=10;
    rankedScore.player+=10;
    showRankedResult('+10 RP','rup');
  }
}

let rrTimeout=null;
function showRankedResult(msg,cls){
  const el=document.getElementById('rrpts');
  el.textContent=msg;el.className='rrpts '+cls;
}

function endRankedRound(){
  rankedRound++;
  const rr=document.getElementById('rankedresult');
  const title=document.getElementById('rrtitle');
  const pts=document.getElementById('rrpts');
  const next=document.getElementById('rrnext');
  const itMost=player.itTime>Math.max(...bots.map(b=>b.itTime));
  title.textContent=itMost?'ROUND LOST':'ROUND WON';
  title.style.color=itMost?'#ff2040':'#ffd700';
  pts.textContent=`Rank Points: ${player.rankPoints} (${getRank(player.rankPoints).name})`;
  pts.style.color=getRank(player.rankPoints).color;
  rr.classList.add('show');
  if(rankedRound>=rankedRoundsTotal){
    next.textContent='MATCH OVER — RETURNING...';
    setTimeout(()=>{rr.classList.remove('show');endGame();},3000);
  } else {
    let cd=3;
    next.textContent=`NEXT ROUND IN ${cd}...`;
    const iv=setInterval(()=>{cd--;if(cd<=0){clearInterval(iv);rr.classList.remove('show');startRankedRound();}else next.textContent=`NEXT ROUND IN ${cd}...`;},1000);
  }
}
...
function startRankedRound(){
  ...
  tagCooldownTimer=0;
  // Scatter positions
  player.x=(Math.random()-.5)*540;player.z=(Math.random()-.5)*540;player.y=0;player.vx=player.vy=player.vz=0;
  bots.forEach(b=>{b.x=(Math.random()-.5)*540;b.z=(Math.random()-.5)*540;b.y=0;b.vx=b.vy=b.vz=0;b.tagIm=0;});
  const itName=all[itIdx].name;
  setTimeout(()=>showStat(itName==='YOU'?'YOU START AS IT!':itName+' IS IT!'),150);
}
...
function updateHUD(dt){
  ...
  tel.className=timer<(gameMode==='ranked'?8:30)?'urg':'';
  ...
  document.getElementById('hrankv').textContent=rk.name+' '+p.rankPoints+'RP';
  document.getElementById('hrankv').style.color=rk.color;
  updSkillSlot('q',p.equippedQ,p.skill1CD);
  updSkillSlot('e',p.equippedE,p.skill2CD);
  // Update bind labels
...
function updateMPHud(){
  ...
  allP.forEach(p=>{
    const d=document.createElement('div');d.className='mphudp';
    const hex='#'+p.color.toString(16).padStart(6,'0');
    d.innerHTML=`<div class="mphudpdot" style="background:${hex}"></div><span class="mphudpname">${p.name}</span>${p.isIt?'<span class="mphudpit">IT</span>':''}`;
    h.appendChild(d);
  });
}

function updSkillSlot(slot,sid,cd){
  const nn=document.getElementById(slot==='q'?'sqn':'sen');
  const no=document.getElementById(slot==='q'?'sqo':'seo');
  const nt=document.getElementById(slot==='q'?'sqt':'set2');
  if(!nn)return;
...
function drawMinimap(){
  const mc=document.getElementById('hmm'),ctx=mc.getContext('2d');
...
function updateCamera(){
  const p=player;
  const eh=p.isSliding?SH*.82:PH*.88;
  const bob=p.isGrounded&&Math.sqrt(p.vx*p.vx+p.vz*p.vz)>.5?Math.sin(p.bobTime)*.044:0;
  camera.position.set(p.x,p.y+eh+bob,p.z);
  camera.rotation.y=p.yaw;camera.rotation.x=p.pitch;camera.rotation.z=p.isDiving?.12:0;
}
...
// ═══════════════════════════════════════════
const lblEls=[];
function initLabels(){lblEls.forEach(l=>l.remove());lblEls.length=0;const all=gameMode==='multi'?multiPlayers:bots;all.forEach(b=>{const el=document.createElement('div');el.className='blbl';document.getElementById('ui').appendChild(el);lblEls.push(el);});}
const tmpV=new THREE.Vector3();
function updateLabels(){
  const all=gameMode==='multi'?multiPlayers:bots;
  all.forEach((b,i)=>{
    const el=lblEls[i];if(!el)return;
    tmpV.set(b.x,b.y+2.45,b.z);tmpV.project(camera);
    if(tmpV.z<1&&tmpV.z>-1){el.style.left=(tmpV.x*.5+.5)*innerWidth+'px';el.style.top=(-.5*tmpV.y+.5)*innerHeight+'px';el.style.display='block';el.textContent=b.isIt?`★ ${b.name} ★`:b.name;el.style.color=b.isIt?'#ff4040':'rgba(255,255,255,.75)';}
...
function addChat(msg,type){chatMsgs.push({msg,type,t:Date.now()});if(chatMsgs.length>7)chatMsgs.shift();updChatDisp();
  if(type!=='sys'&&Math.random()<.3)setTimeout(()=>{const all=gameMode==='multi'?multiPlayers:bots;if(all.length)addChat(`${all[Math.floor(Math.random()*all.length)].name}: ${BOTLINES[Math.floor(Math.random()*BOTLINES.length)]}`,'bot');},600+Math.random()*1800);}
...
function startGame(map,mode){
  ...
  const sp=[[225,0,0],[-225,0,0],[0,0,-225]];
  ...
  // Setup multiplayer players
  if(gameMode==='multi'){
    const n=Math.floor(Math.random()*3)+2; // 2-4 simulated others
    const colors=[0xff6644,0x44ff88,0x4488ff,0xffdd44,0xff44ff];
    multiPlayers=Array.from({length:n},(_,i)=>({
      id:i,name:HUMAN_NAMES[i+Math.floor(Math.random()*4)],
      x:(Math.random()-.5)*18,y:0,z:(Math.random()-.5)*18,
      vx:0,vy:0,vz:0,isIt:false,isGrounded:true,itTime:0,tagsMade:0,
      mesh:null,aiAng:Math.random()*Math.PI*2,stuckT:0,stuckCk:0,
      pX:0,pZ:0,tagIm:0,color:colors[i+1],aiTimer:0
    }));
  } else multiPlayers=[];

  const all=gameMode==='multi'
    ?[{ref:player,name:'YOU'},...multiPlayers.map(b=>({ref:b,name:b.name}))]
    :gameMode==='practice'?[{ref:player,name:'YOU'}]
    :[{ref:player,name:'YOU'},...bots.map(b=>({ref:b,name:b.name}))];
  const itIdx=Math.floor(Math.random()*all.length);all[itIdx].ref.isIt=true;

  chatMsgs.length=0;
  buildScene(curMap);initLabels();
  showScreen('hud');
...
function endGame(){
  ...
  const all=gameMode==='multi'
    ?[{name:'YOU',itTime:player.itTime,tagsMade:player.tagsMade,isPlayer:true},...multiPlayers.map(b=>({name:b.name,itTime:b.itTime,tagsMade:b.tagsMade,isPlayer:false}))]
    :gameMode==='practice'?[{name:'YOU',itTime:player.itTime,tagsMade:player.tagsMade,isPlayer:true}]
    :[{name:'YOU',itTime:player.itTime,tagsMade:player.tagsMade,isPlayer:true},...bots.map(b=>({name:b.name,itTime:b.itTime,tagsMade:b.tagsMade,isPlayer:false}))];
  ...
  if(gameMode==='ranked'){
    if(playerLost){player.rankPoints=Math.max(0,player.rankPoints-25);}
    else{player.rankPoints+=50;}
  } else if(gameMode!=='practice'){
    if(!playerLost)player.coins+=1;
  }
  ...
  leaderboard.sort((a,b)=>(b.wins-b.losses)-(a.wins-a.losses));
  doSave();
  buildResultScreen(all,playerLost,prevRank,newRank);
  if(player.matchPoints>=10)setTimeout(()=>showScreen('upg'),3000);
...
function buildResultScreen(ranked,lost,prevRank,newRank){
  ...
  ranked.forEach(r=>{
    const div=document.createElement('div');div.className='rp'+(r.isPlayer?' you':'');
    const prev=r.isPlayer?(lost?-1:1):(r.name===ranked[0].name?-1:1);
    const cls=prev>0?'rc rup':prev<0?'rc rdn':'rc req';
    div.innerHTML=`<span class="pn">${r.isPlayer?'YOU':r.name}</span><span class="${cls}">${r.tagsMade} tags</span><span style="color:#666;font-size:.75rem">${r.itTime.toFixed(1)}s IT</span>`;
    rp.appendChild(div);
  });
  // Rank change
  const rc=document.getElementById('rankchange');
  if(gameMode==='ranked'){
    const rk=getRank(player.rankPoints);
    if(prevRank!==newRank)rc.innerHTML=`<span class="${lost?'rankloss':'rankgain'}">${lost?'▼ RANK DOWN':'▲ RANK UP'}: ${prevRank} → <span style="color:${rk.color}">${newRank}</span></span>`;
    else rc.innerHTML=`<span style="color:${rk.color}">${rk.name} — ${player.rankPoints} RP</span>`;
  } else rc.innerHTML='';
}
...
function showScreen(name){
  gScreen=name;
  SCREENS.forEach(s=>document.getElementById('s-'+s).classList.add('hide'));
...
function drawMapPreview(map){
  const c=document.getElementById('mprevcanvas');
  const ctx=c.getContext('2d');
  const W=300,H=168;
  const th=map.theme;
  ctx.fillStyle='#'+th.sky.toString(16).padStart(6,'0');ctx.fillRect(0,0,W,H);
  // Ground strip
  ctx.fillStyle='#'+th.gr.toString(16).padStart(6,'0');ctx.fillRect(0,H*.7,W,H*.3);
  // Draw obstacles as 2D top-down
  const sc=W/MSZ,cx=W/2,cy=H/2;
  for(const o of map.obs){
    if(o.type==='bound')continue;
    let col=th.w;
    if(o.type==='slide')col=0x00ffcc;
    else if(o.type==='jump')col=0xffaa00;
    else if(o.type==='barrier')col=0xff4400;
    ctx.fillStyle='#'+col.toString(16).padStart(6,'0');
    ctx.globalAlpha=o.type==='wall'||o.type==='platform'?.7:.9;
    ctx.fillRect(cx+o.mnX*sc,cy-o.mxZ*sc,(o.mxX-o.mnX)*sc,(o.mxZ-o.mnZ)*sc);
  }
  ctx.globalAlpha=1;
  // Player spawn
  ctx.fillStyle='#ffd700';ctx.beginPath();ctx.arc(cx,cy,4,0,Math.PI*2);ctx.fill();
  // Boundary
  ctx.strokeStyle='#'+th.aw.toString(16).padStart(6,'0');ctx.lineWidth=1.5;ctx.globalAlpha=.5;
  ctx.strokeRect(cx-HM*sc,cy-HM*sc,MSZ*sc,MSZ*sc);ctx.globalAlpha=1;
}

function showMapPreview(map){
  document.getElementById('mprevname').textContent=map.n;
  document.getElementById('mprevtheme').textContent=map.t.toUpperCase();
  document.getElementById('mprevlayout').textContent=map.p.toUpperCase();
  const diff=Math.min(5,Math.round(map.d/3+map.s*5));
  document.getElementById('mprevdiff').textContent='★'.repeat(diff)+'☆'.repeat(5-diff);
  document.getElementById('mprevobs').textContent=map.obs.filter(o=>o.type!=='bound').length+' obstacles';
  document.getElementById('mprevdesc').textContent=THEME_DESCS[map.t]||'';
  drawMapPreview(map);
...
function openMapSelect(mode){
  pendingMode=mode||'casual';
  const pool=[...MAPS];
  mapChoices=[];
  const num=Math.min(5,pool.length);
  while(mapChoices.length<num){const i=Math.floor(Math.random()*pool.length);mapChoices.push(pool.splice(i,1)[0]);}
  selMapIdx=-1;
  const con=document.getElementById('mcards');con.innerHTML='';
  document.getElementById('btn-go').style.display='none';
  document.getElementById('mprevname').textContent='← PICK A MAP';
  document.getElementById('mprevtheme').textContent='—';
  document.getElementById('mprevlayout').textContent='—';
  document.getElementById('mprevdiff').textContent='—';
  document.getElementById('mprevobs').textContent='—';
  document.getElementById('mprevdesc').textContent='Click any map card on the left to preview it here.';
  ...
  mapChoices.forEach((map,i)=>{
    ...
    const th=map.theme;
    const bg=document.createElement('canvas');bg.className='mcbg';bg.width=185;bg.height=250;
    const bctx=bg.getContext('2d');
    bctx.fillStyle='#'+th.sky.toString(16).padStart(6,'0');bctx.fillRect(0,0,185,250);
    bctx.fillStyle='#'+th.gr.toString(16).padStart(6,'0');bctx.fillRect(0,190,185,60);
    bctx.fillStyle='#'+th.w.toString(16).padStart(6,'0');
    const rng=mkRng(map.seed+9999);
    for(let j=0;j<10;j++){bctx.fillRect(rng()*168+5,rng()*168+20,8+rng()*22,6+rng()*55);}
    card.appendChild(bg);
    const diff=Math.min(5,Math.round(map.d/3+map.s*5));
    ...
    card.addEventListener('click',()=>{
      document.querySelectorAll('.mc').forEach(c=>c.classList.remove('sel'));card.classList.add('sel');
      selMapIdx=i;showMapPreview(mapChoices[i]);
    });
...
function openLobby(){
  lobbyRoomCode=Math.random().toString(36).substr(2,6).toUpperCase();
  lobbyMap=MAPS[Math.floor(Math.random()*MAPS.length)];
  lobbyPlayerCount=1;
  document.getElementById('lobbycode').textContent=lobbyRoomCode;
  renderLobbySlots();
  showScreen('lobby');
...
function simulateLobbyJoins(){
  const n=Math.floor(Math.random()*3)+1; // 1-3 join
  let joined=0;
  const iv=setInterval(()=>{
    if(joined>=n||!document.getElementById('s-lobby').classList.contains('hide')===false){clearInterval(iv);return;}
    if(gScreen!=='lobby'){clearInterval(iv);return;}
    lobbyPlayerCount++;joined++;
    renderLobbySlots();
    if(lobbyPlayerCount>=2){document.getElementById('btn-lobbystart').style.display='block';}
    document.getElementById('lobbyinfo').textContent=`${lobbyPlayerCount} PLAYER${lobbyPlayerCount>1?'S':''} IN LOBBY — ${lobbyPlayerCount>=2?'READY TO START!':'WAITING...'}`;
...
function renderLobbySlots(){
  ...
  for(let i=0;i<max;i++){
    ...
    const col='#'+LOBBY_COLORS[i].toString(16).padStart(6,'0');
    if(i<lobbyPlayerCount){
      div.className='pslot '+(i===0?'you':'filled');
      div.innerHTML=`<div style="width:12px;height:12px;border-radius:50%;background:${col};margin-bottom:2px"></div><div class="pslotname">${i===0?'YOU':HUMAN_NAMES[i-1]}</div>${i===0?'<div class="pslotyou">HOST</div>':'<div class="pslotping">${Math.floor(Math.random()*60+10)}ms</div>'}`;
    } else {
      div.className='pslot empty';
      div.innerHTML=`<div style="color:#333;font-size:.7rem">WAITING...</div>`;
    }
...
function buildShopUI(){
  ...
  SDEFS.filter(def=>!def.builtin).forEach(def=>{
    const owned=player.ownedSkills.includes(def.id);
    const isEqQ=player.equippedQ===def.id,isEqE=player.equippedE===def.id;
    ...
    div.addEventListener('click',()=>{
      if(!owned){if(player.coins<def.cost){showStat('NOT ENOUGH COINS!');return;}player.coins-=def.cost;player.ownedSkills.push(def.id);}
      if(def.slot==='q')player.equippedQ=def.id;else player.equippedE=def.id;
      doSave();buildShopUI();
    });
    grid.appendChild(div);
  });
  // Add "reset Q to slide" option
  ...
  // Keybind UI
  document.getElementById('kb-q').textContent=BINDS.q.replace('Key','').replace('Digit','').replace('Arrow','↑').replace('Left','←').replace('Right','→');
  document.getElementById('kb-e').textContent=BINDS.e.replace('Key','').replace('Digit','').replace('Arrow','↑');
}

// Keybind listeners
document.querySelectorAll('.kbkey').forEach(btn=>{
  btn.addEventListener('click',()=>{
    listenBind=btn.dataset.action;
    document.querySelectorAll('.kbkey').forEach(k=>k.classList.remove('listening'));
...
function buildUpgradeUI(){
  ...
  UDEFS.forEach(def=>{
    const lvl=player.upgrades[def.id]||0;const maxed=lvl>=def.max;
...
function buildLB(){
  ...
  sorted.forEach((e,i)=>{
    const rk=getRank(e.rp||0);
    const div=document.createElement('div');div.className='lbr'+(e.name==='YOU'?' you':'');
    div.innerHTML=`<span>#${i+1}</span><span>${e.name}</span><span>${e.wins}</span><span>${e.losses}</span><span>${e.streak}</span><span style="color:${rk.color};font-size:.72rem">${rk.name}</span>`;
...
document.getElementById('btn-lobbystart').addEventListener('click',()=>{if(lobbyMap)startGame(lobbyMap,'multi');else openMapSelect('multi');});
...
document.getElementById('btn-joinroom').addEventListener('click',()=>{const code=document.getElementById('joincode').value.trim().toUpperCase();if(code.length>=4){lobbyRoomCode=code;document.getElementById('lobbycode').textContent=code;lobbyPlayerCount=2;renderLobbySlots();document.getElementById('btn-lobbystart').style.display='block';document.getElementById('lobbyinfo').textContent='JOINED ROOM '+code+' — 2 PLAYERS READY';}});
...
function makeWebTexture(){
  ...
  for(let i=0;i<spokes;i++){
    ...
    ctx.lineTo(cx+Math.cos(a)*cx*.96,cy+Math.sin(a)*cy*.96);ctx.stroke();
  }
  // Concentric ring threads
  const rings=9;
  for(let r=1;r<=rings;r++){
    const rad=(r/rings)*cx*.94;
    ctx.beginPath();
    for(let i=0;i<=spokes;i++){
      const a=i/spokes*Math.PI*2;
      const jitter=1+(Math.sin(i*7.3+r*3.1)*.05);
      const rx=cx+Math.cos(a)*rad*jitter,ry=cy+Math.sin(a)*rad*jitter;
  ...
  for(let i=0;i<spokes;i++){for(let r=1;r<=rings;r++){
    if(Math.random()>.55)continue;
    const a=i/spokes*Math.PI*2,rad=(r/rings)*cx*.94;
    ctx.beginPath();ctx.arc(cx+Math.cos(a)*rad,cy+Math.sin(a)*rad,1.5,0,Math.PI*2);ctx.fill();
  }}
  const tex=new THREE.CanvasTexture(cv);return tex;
}
const _webTex=null;
function getWebTex(){if(!getWebTex._t)getWebTex._t=makeWebTexture();return getWebTex._t;}

function buildWebMesh(obs,sc){
  const target=sc||scene;
  const tex=getWebTex();
  const mat=new THREE.MeshBasicMaterial({map:tex,transparent:true,opacity:0.82,side:THREE.DoubleSide,depthWrite:false});
  // Three planes forming a volumetric web cluster
  const planes=[
    [obs.w,obs.h,0,0],   // XY face
    [obs.d,obs.h,Math.PI/2,0], // ZY face rotated
    [obs.w,obs.d,0,Math.PI/2], // XZ face (horizontal)
  ];
  const cy=obs.mnY+obs.h*.5;
  planes.forEach(([pw,ph,ry,rx])=>{
    const pm=new THREE.Mesh(new THREE.PlaneGeometry(pw,ph),mat.clone());
    pm.position.set(obs.x,cy,obs.z);
    pm.rotation.y=ry;pm.rotation.x=rx;
    target.add(pm);
  });
  // Subtle glow sphere
  const glow=new THREE.Mesh(
    new THREE.SphereGeometry(Math.max(obs.w,obs.h,obs.d)*.45,8,6),
    new THREE.MeshBasicMaterial({color:0xaaddff,transparent:true,opacity:0.06,side:THREE.BackSide})
  );
...
function loop(now){
  ...
  if(timer<=0){
    timer=0;
    if(gameMode==='ranked'){endRankedRound();}
    else{endGame();}
    return;
  }
  updatePlayer(dt);updateBots(dt);checkTag();
  updateCamera();updateHUD(dt);updateLabels();
  if(decoyMesh&&decoyMesh.visible)decoyMesh.rotation.y+=dt*3;
  bots.forEach(b=>{const r=b.mesh?.getObjectByName('itr');if(r&&r.visible)r.rotation.y+=dt*2;});
  multiPlayers.forEach(b=>{const r=b.mesh?.getObjectByName('itr');if(r&&r.visible)r.rotation.y+=dt*2;});
